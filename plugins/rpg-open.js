...
let handler = async (m, { command, args, usedPrefix }) => {
    // Pengecekan cooldown 5 menit
    let lastUsage = global.db.data.users[m.sender].lastUsage || 0
    let elapsedTime = Date.now() - lastUsage
    let remainingTime = 300000 - elapsedTime // 5 menit dalam milidetik
    if (elapsedTime < 300000) {
        let time = clockString(remainingTime)
        return await conn.reply(m.chat, `Hanya dapat digunakan lagi setelah *${time}*`, m)
    }
    ...
    let thumb2 = 'https://telegra.ph/file/e2ede01a66810a4355bdd.jpg'
...
`.trim()
    ...
    let count = Math.floor(isNumber(args[1]) ? Math.min(Math.max(parseInt(args[1]), 1), 99999) : 1) * 1
    ...
    if (count >= 100000) throw 'Max: 99999'
...
`.trim()).join('\n')}
`.trim()
    await conn.reply(m.chat, opened, m, { mentionedJid: [m.sender], contextInfo: { forwardingScore: 9999, isForwarded: true, externalAdReply: { mediaType: 1, mediaUrl: thumb2, title: `${htki} OPENED< CRATES ${htka}`, body: '🌱┊ RPG WhatsApp Bot', thumbnail: { url: thumb2 }, thumbnailUrl: thumb2, sourceUrl: null, renderLargerThumbnail: true }}})
